...
class TurtleBot3Env(robot_gazebo_env.RobotGazeboEnv):
    ...
    def __init__(self, ros_ws_abspath):
        ...
        * /robot_bumper: Contacts sensor
        ...
        ROSLauncher(rospackage_name="spawn_robots",
                    launch_file_name="spawn_turtlebot3_traffic_light.launch",
    ...
    def _check_all_sensors_ready(self):
        ...
        self._check_camera_image_ready()
        self._check_contacts_state_ready()
    ...
    def _check_contacts_state_ready(self):
        ...
        rospy.logdebug("Waiting for /robot_bumper to be READY...")
        while self.contacts_state is None and not rospy.is_shutdown():
            try:
                self.contacts_state = rospy.wait_for_message("/robot_bumper", ContactsState, timeout=1.0)
                rospy.logdebug("Current /robot_bumper READY=>")

            except:
                rospy.logerr("Current /robot_bumper not ready yet, retrying for getting contacts_state")
        return self.contacts_state
    ...
    def _camera_image_callback(self, data):
        self.camera_image = data

    def _contacts_state_callback(self, data):
        self.contacts_state = data
